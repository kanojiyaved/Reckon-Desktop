...
# Load data
df = pd.read_csv("/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /ALL DATA/All Data - All Data.csv.csv", low_memory=False)
...
df_Job = pd.read_csv("/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /JOB File /Reckon Desktop Job - Sheet1.csv")
...
# Clean df
df = df[(df['Type'] == 'Tax Invoice') & (df['Account Type'] != 'Accounts Receivable')]
...
df["Unit Price"] = df["Amount ($)"] * original_sign.map({1: -1, -1: 1})
...
df["Invoice number"] = np.random.randint(100000, 999999, size=len(df)).astype(str)

# Reorder columns
columns_order = [
    "Invoice number", "Customer", "Transaction date", "Due date", "Customer PO No.",
    "Amounts are", "Item", "Description", "Account No.", "No. of Unit",
    "Unit Price", "Discount %", "Amount ($)", "Tax code", "Tax amount ($)", 
    "Job No.", "Job name"
]
...
# Export
output_path = r"/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /ALL DATA/All Data - All Data.csv - Invoice.xlsx"
