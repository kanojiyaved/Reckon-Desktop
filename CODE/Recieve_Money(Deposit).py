...
# Load data
df = pd.read_csv("/Users/vedantkanojiya/Desktop/Vedant kanojiya/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /Recieve Money(Deposit)/All Data - All Data.csv.csv")
df_COA_mapping = pd.read_csv("/Users/vedantkanojiya/Desktop/Vedant kanojiya/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /Spend Money(cheque)/COA - MAPPING NEW.xlsx - Sheet1-2.csv")
...
# Filter Cheque rows
df = df[(df['Type'] == 'Deposit') & (df['Account Type'] != 'Accounts Receivable')]
df = df[~df['Account'].str.contains("Deposit", na=False)]
...
# Export to Excel
output_path = r"/Users/vedantkanojiya/Desktop/Vedant kanojiya/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /Recieve Money(Deposit)/MYOB - Recieve Money (Deposit).xlsx"
df.to_excel(output_path, sheet_name="Deposit", index=False)
