...
# Load data
df = pd.read_csv("/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /BILL/All Data - All Data.csv.csv")
df_COA_mapping = pd.read_csv("/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/CODE/COA/Coa Mapping - Sheet1.csv")
...
# Set Bill number = Supplier invoice number
df["Bill number"] = df["Supplier invoice number"]
...
# Remove rows where Unit Price is 0 or NaN
df = df[df["Unit Price"].notna() & (df["Unit Price"] != 0)]
...
# Map Job No. and Job name
df["Job name"] = df["Class"]
df["Job No."] = df["Class"].map(Job_mapping)
...
# Final column order
columns_order = [
    "Bill number", "Supplier", "Transaction date", "Due date", "Supplier invoice number", "Amounts are", "Item",
    "Description", "Account No.", "No. of Unit", "Unit Price", "Discount %", "Amount ($)", "Tax code", "Tax amount ($)",
    "Job No.", "Job name"
]
...
# Export to Excel
output_path = r"/Users/vedantkanojiya/Desktop/MMC INTERNSHIP/RECKON DESKTOP TO MYOB/RECKONS FILES /BILL/MYOB - BILL.xlsx"
df.to_excel(output_path, sheet_name="Bill", index=False)

print(df.head())
print("✅ Final output created: commas removed, jobs mapped, Unit Price cleaned.")
